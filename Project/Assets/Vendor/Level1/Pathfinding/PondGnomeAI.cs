...
public class PondGnomeAI : GnomeAI {
	...
	public override bool Find()
	{
		if(!foundObject)
		{
			...
			foreach( GameObject rockSugar in GameObject.FindGameObjectsWithTag("RockSugar"))
			{
				//If flour exists, and it hasn't been collected yet, then the gnome should collect it
				if(rockSugar != null && rockSugar.transform.parent == null && Vector3.Distance(transform.position, rockSugar.transform.position) <= 60)
				{
					objToCollect = rockSugar;
	...
	{
		//If rocksugar exists, and it is mine then release it
